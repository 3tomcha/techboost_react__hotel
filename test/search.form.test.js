...
import SearchForm from '../src/js/Components/SearchForm.js';
...
it("if all input is '' and submited , then do alert" , () => {
  // アラートをモックとして取得する
  ...
  const p = container.querySelector('.button');
  ReactTestUtils.Simulate.click(p);
  // アラートが呼ばれたかを判定する。呼ばれていなかったらテスト失敗になる
  expect(window.alert).toHaveBeenCalled();
});

it("checkInDay and checkOutDay must be set in input", () => {
  act(() => {
    ...
    ReactDOM.render(
      ...
      expect(input_in.value).toBe("07/04/2019");

      // チェックアウト日の入力ボックス
      let input_out = container.querySelectorAll('input')[2];
      expect(input_out.value).toBe("07/04/2019");
    });
